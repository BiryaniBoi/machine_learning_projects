def flood_fill(image, start, newColor):
    ...
    initialColor = getPixel(image, start[0], start[1])
    queue = [start]
    visited = set()
    while len(queue) > 0:
        #location = queue.pop(0) #BFS
        location = queue.pop() #DFS
        row, col = location
        if location not in visited:
            setPixel(image, row, col, newColor)
            visited.add(location)
            neighbors = getNeighbors(image, row, col)
            for neighbor in neighbors:
                if getPixel(image, neighbor[0], neighbor[1]) == initialColor:
                    queue.append(neighbor)

def getNeighbors(image, row, col):
    res = []
    for r in range(row-1, row+2):
        for c in range(col-1, col+2):
            if 0 <= r < getHeight(image) and 0 <= c < getWidth(image):
                res.append((r,c))
    return res
...
SCALE = 7
IMAGE = "resources/flood_input.png"
